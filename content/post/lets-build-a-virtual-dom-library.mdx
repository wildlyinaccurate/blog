...
0. [Reconciliation, or patching the DOM](lets-build-a-virtual-dom-library/reconciliation-or-patching-the-dom)
0. [What about JSX?](lets-build-a-virtual-dom-library/what-about-jsx)
...
The lifecycle of a virtual DOM library is therefore quite simple: some web application has components that render a VDOM tree. The VDOM library mounts or patches that tree into the real DOM. The user interacts with the real DOM, triggering events that the web application reacts to, which starts the whole cycle again.
