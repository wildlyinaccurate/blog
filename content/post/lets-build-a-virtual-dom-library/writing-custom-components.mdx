...
This is not completely unexpected. So far our `mount()` function can only handle vnodes that have a valid HTML tag name as their `type` property. To handle components, we'll need to first call their `render()` function until we end up with a regular vnode. In JavaScript classes are actually just functions, so we can add an `if (typeof vnode.type === "function")` branch to handle components.
